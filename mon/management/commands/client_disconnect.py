...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError("Common name %s has not been enabled to access the VPN" % common_name)

        try:
            log_entry = OpenVPNLog.objects.filter(common_name=common_name, when_disconnect__isnull=True).latest()
            log_entry.bytes_sent = int(os.environ['bytes_sent'])
            log_entry.bytes_received = int(os.environ['bytes_received'])
            log_entry.when_disconnect = datetime.datetime.fromtimestamp(int(os.environ['time_duration']) + int(os.environ['time_unix']))
        except KeyError:
            raise CommandLogError(NO_ENV_MESSAGE)
