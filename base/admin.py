...
class ClientForm(forms.ModelForm):
    ...
    def clean_name(self):
        ...
        if cnames.count() > 1 or (cnames.count() == 1 and self.instance.pk != cnames[0].pk):
...
class ClientAdmin(admin.ModelAdmin):
    ...
    form = ClientForm
    save_on_top = True
    list_display = ('subnet_with_link',
        'company', 'name', 'common_name', 'ip',
    ...
    actions = ['create_cert', 'show_cert', 'revoke_cert', 'publish_cert', 'download_cert']
    search_fields = ('name', 'common_name', 'ip', 'company')


    def subnet_with_link(self, obj):
        url = urlresolvers.reverse('admin:base_vpnsubnet_change', args=(obj.subnet.id,))
        ...
        for obj in queryset.all():
            if obj.cert:
                messages.error(request,
                    _("You cannot create a certificate for %s because it has already one") % obj)
        ...
        if not has_cert:
            ...
            for obj in queryset.all():
                try:
                    ...
                    messages.error(request,
                        _("Certificate creation for %(obj)s failed, error: %(e)s") % {
                            'obj' : obj, 'e' : e
                        }
                    )
    ...
    def show_cert(self, request, queryset):
        ...
        url = u"%s?ct=%s&ids=%s" % (
            urlresolvers.reverse("display-certs"),
            ct.pk, ",".join(selected)
        )
    ...
    def publish_cert(self, request, queryset):
        ...
        for el in queryset:
            if not el.cert:
                messages.error(request, _("No certificate has been created for client %s, cannot publish") % el)
                url = urlresolvers.reverse('admin:base_client_changelist')
                verify_certs = False
        ...
        if verify_certs:
            ...
            ct = ContentType.objects.get_for_model(queryset.model)
            url = u"%s?ct=%s&ids=%s" % (
                urlresolvers.reverse("display-distributions"),
                ct.pk, ",".join(selected)
            )
    ...
    def download_cert(self, request, queryset):
        selected = request.POST.getlist(admin.ACTION_CHECKBOX_NAME)
        if len(selected)>1:
            messages.error(request, _("You can download only certificates for one client a time"))
            url = urlresolvers.reverse('admin:base_client_changelist')
        elif not queryset[0].cert:
            messages.error(request, _("No certificate has been created for this client"))
            url = urlresolvers.reverse('admin:base_client_changelist')
        else:
            url = urlresolvers.reverse('private-cert-download', kwargs={ "client_id" : int(selected[0])})
        return HttpResponseRedirect(url)
    ...
    fieldsets = (
        (None, {
            'fields' : (('name', 'human_name'),
                        ('base', 'bits'), 'default_gw',
                        ('static_min','static_max'),
            )
         }),
         ('VPN settings', {
            'fields' : ('topology', 'bound_iface', 'config_server', 'config_client')
         }),
         ("Extra", {
