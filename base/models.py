...
PKITOOL = 'cd %s; . ./vars; ./pkitool %s' % (settings.EASY_RSA_DIR, "%s")
REVOKEFULL = 'cd %s; . ./vars; ./revoke-full %s' % (settings.EASY_RSA_DIR,
                                                    "%s")
...
class ClientActionsLog(models.Model):
    ...
    action = models.CharField(max_length=32, choices=ACTIONS_LIST)
    ...
    def __unicode__(self):
        return "%s %s on %s" % (self.client, self.action, self.on)
