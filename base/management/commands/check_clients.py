        ...
        if not l:
            ...
            if l != clients.count():
                raise CommandError("One or more clients are not found.\n" +
                    "Clients found are %s." % " ".join(map(unicode, clients))
                )
        ...
        for client in clients:
            ...
            if not client.cert:
                if client.exist_cert():
                    log.warning("Cert for %s exist, but not bound to client" % client.common_name)
                    client.bind_cert()
                    client.save()
                    log.info("Cert for %s bound to client" % client.common_name)
