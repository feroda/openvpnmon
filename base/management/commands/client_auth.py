...
from openvpnmon.base.models import Client, ACTION_CLIENT_AUTHORIZATION_UPDATE, ACTION_ERROR_CLIENT_AUTHORIZE
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError("You should specify a common_name af an already existent host in OpenVPNmon database")
        ...
        log.info(u"Removing rules for %s: iface %s, ip: %s" % (client, client.subnet.bound_iface, client.ip))
        try:
            out = call_shell(shell_cmd)
        except CalledShellCommandError as e:
            error_log = "%s | return code %s | %s" % (e.shell_cmd, e.returncode, e.output)
            log.debug(error_log)
            client_log = ClientActionsLog(action=ACTION_ERROR_CLIENT_AUTHORIZE, note=error_log)
